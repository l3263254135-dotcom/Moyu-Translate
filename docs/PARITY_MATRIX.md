...
| 系统朗读 | macOS `say`/系统声音 | Windows Speech | 英文单词/短语查询后可自动朗读；开关、手动朗读和重复查询行为一致 |
| 主释义排版 | React 共享组件 | React 共享组件 | 相同宽度下视觉快照一致 |
| 学习内容 | 音标、词形、例句、关系 | 相同 | 默认紧凑，展开后不溢出 480 点面板 |
| 生词本与卡片复习 | 本地 SQLite | 本地 SQLite | 旧收藏无损迁移；到期、评分和重启后进度一致，不联网 |
...
- `v0.2.0-beta.5` Tauri 共享 UI、Dictionary v2、生词本卡片复习、查询后自动发音、异常输入保护、查询取消、菜单栏/托盘入口和已校验模型缓存：已建立。
- macOS Option 监听增加权限反馈、500ms/1s/2s 自动重试和 CGEventTap 禁用恢复；Windows Alt 行为保持不变。
